            ...
            FROM
                          (
                             ...
                             WHERE
                                           O.OPEN_ORDER_IND = 'Y'
                                           AND O.PO_TYPE_ID <>'RO'
                                           AND O.SNAP_DT BETWEEN DATE-4 AND DATE-1 
                          ...
                          FULL OUTER JOIN
                                         (
                                           ...
                                           WHERE
                                                          D.SNAP_DT BETWEEN DATE-4 AND DATE-1
...
def prep_cat_level():  # old model
    ...
    fourdaysago = datetime.strftime(datetime.now() - timedelta(days=4), '%Y-%m-%d')
    ...
    part_c = df[df.index == fourdaysago]
    # make sure we only look at current month

    # make sure there is no missing lines comparing yesterday vs today
    if df.index[-1].day > 3 and df.index[-1].dayofweek == 6:
        check_point = pd.merge(part_a, part_c, on=['CTGY'], how='outer', suffixes=('', '_db')).fillna(0)
        part_a = check_point[[col for col in check_point if 'db' not in col]]
        part_c = check_point[[col for col in check_point if 'db' in col]]
        part_c.columns = part_c.columns.str.rstrip('_db')
    else:
        check_point = pd.merge(part_a, part_b, on=['CTGY'], how='outer', suffixes=('', '_db')).fillna(0)
        part_a = check_point[[col for col in check_point if 'db' not in col]]
    ...
    if df.index[-1].day > 1:
        # check if it isa Monday and also dayofmonth is greater than 2 then we compare with Friday
        if df.index[-1].dayofweek == 6 and df.index[-1].day > 3:
            part_a.loc[:, 'VS_Friday'] = (part_a['shipped_plus_working'].values -
                                          part_c['shipped_plus_working'].values).astype('int64')
            ...
            part_a = part_a[['CTGY', name_sop, 'shipped_plus_working', f'VS {name_sop}', 'VS_Friday']]
        else:
...
def prep_customer_level():
    ...
    part_c = df[df.index == fourdaysago]
    # make sure we only look at current month
    # make sure there is no missing customers comparing yesterday vs today
    if df.index[-1].day > 3 and df.index[-1].dayofweek == 6:
        check_point = pd.merge(part_a, part_c, on=['CUSTOMER'], how='outer', suffixes=('', '_db')).fillna(0)
        part_a = check_point[[col for col in check_point if 'db' not in col]]
        part_c = check_point[[col for col in check_point if 'db' in col]]
        part_c.columns = part_c.columns.str.rstrip('_db')
    else:
        check_point = pd.merge(part_a, part_b, on=['CUSTOMER'], how='outer', suffixes=('', '_db')).fillna(0)
        part_a = check_point[[col for col in check_point if 'db' not in col]]
        part_b = check_point[[col for col in check_point if 'db' in col]]
        part_b.columns = part_b.columns.str.rstrip('_db')
    ...
    if df.index[-1].day > 1:
        ...
        if df.index[-1].dayofweek == 6 and df.index[-1].day > 3:
            ...
            part_a.fillna(value=0, inplace=True)
            part_a[f'VS {name_sop}'] = (part_a['shipped_plus_working'] - part_a[name_sop]).astype('int64')
            part_a = part_a[['CUSTOMER', name_sop, 'shipped_plus_working', f'VS {name_sop}', 'VS_Friday']]
        else:
            part_a.loc[:, 'VS_Prior_Day'] = (part_a['shipped_plus_working'].values -
                                             part_b['shipped_plus_working'].values).astype('int64')
            part_a = part_a.merge(plan[['SOP_QTY', 'CUSTOMER']], left_on='CUSTOMER',
...
def prep_line_level():
    ...
    # make sure there is no missing lines comparing yesterday vs today
    if df.index[-1].day > 3 and df.index[-1].dayofweek == 6:
        check_point = pd.merge(part_a, part_c, on=['CUSTOMER', 'LINE'], how='outer', suffixes=('', '_db')).fillna(0)
        part_a = check_point[[col for col in check_point if 'db' not in col]]
        part_c = check_point[[col for col in check_point if 'db' in col]]
        part_c.columns = part_c.columns.str.rstrip('_db')
    else:
    ...
    if df.index[-1].day > 1:
        if df.index[-1].dayofweek == 6 and df.index[-1].day > 3:
            ...
            part_a.fillna(value=0, inplace=True)
            part_a[f'VS {name_sop}'] = (part_a['shipped_plus_working'] - part_a[name_sop]).astype('int64')
            part_a = part_a[['CUSTOMER', 'LINE', name_sop, 'shipped_plus_working', f'VS {name_sop}', 'VS_Friday']]
            part_a.sort_values(by='VS_Friday')
        else:
            part_a.loc[:, 'VS_Prior_Day'] = (part_a['shipped_plus_working'].values -
                                             part_b['shipped_plus_working'].values).astype('int64')
            part_a = part_a.merge(plan_CT[['SOP_QTY', 'CUSTOMER', 'LINE']], left_on=['CUSTOMER', 'LINE'],
...
def line_mail():
    ...
    if datetime.now().day > 4 and datetime.now().weekday() == 0:
        df = df.sort_values(by='VS_Friday', ascending=False)
        df = pd.concat([df.head(5), df.tail(5)])
        df = df[abs(df['VS_Friday']) > 0]
