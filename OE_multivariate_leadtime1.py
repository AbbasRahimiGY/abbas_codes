...
class Models:
    ...
    def xgb(self,lead_time):
        ...
        plt.axis('tight')
        fig1.savefig('xgboost_{}.png'.format(lead_time))
        if forecast < 0:
            forecast[0] = 0
...
def prep_data(data, holiday):
    ...
    df.drop(columns=['day_left', 'day'], inplace=True)
    # different type of lags
    df['yday_ship'] = df.groupby(['CUSTOMER'])['SHIP_QTY'].shift(1).values
    ...
    df['is_holiday'] = np.where(df.index.isin(holiday_nd.ds), 1, 0)
...
def data_resampling(df, df_shipment, current):
    ...
    for index, row in df.iterrows():
        ...
        df.loc[index, 'forecast_low'] = np.where(round(np.percentile(samples, 2.5), 0) > 0,
                                                 round(np.percentile(samples, 2.5), 0), 0)
        df.loc[index, 'forecast_high'] = np.where(round(np.percentile(samples, 97.5), 0) > 0,
                                                  round(np.percentile(samples, 97.5), 0), 0)
    df.drop(columns=['mean', 'std'],inplace=True)
    df.to_excel('daily_forecast_{}.xlsx'.format(current))
    # now merge with current month shipment
    df = df.groupby('CUSTOMER')['forecast_mean', 'forecast_low','forecast_high'].sum()
    mtd_ship = df_shipment.copy().rename(columns={'SHIP_QTY': 'MTD_shipped'})
...
if __name__ == '__main__':
    ...
    type = 'ppparallel'
    # -----------------------
    exogenous_features = ['IP','CON', 'month',
                          'past_seven', 'past_twentyeight', 'is_holiday']

    scale_list = ['y']

    max_terms = len(exogenous_features)
    cutoff = '2020-03-14'  # where to cutoff data
    test = '2020-03-15'  # where we sample from snap date for all horizons
    result = []
    for lead in range(1, 17):
        ...
        if type == 'parallel':
            ...
            for cus in orders.CUSTOMER.unique():
                ...
                yhat_cus = my_model.xgb(lead_date)  # mars(max_degree=1)
                if yhat_cus > 0:
                    y_hat += yhat_cus
